-- Grade every Marugoto test from its Supabase section definition.
-- Answer keys stay server-side and client-provided scores are ignored.

CREATE OR REPLACE FUNCTION calculate_marugoto_scores(
  p_test_id VARCHAR,
  p_answers JSONB
)
RETURNS JSONB
...
BEGIN
  IF p_test_id NOT LIKE 'marugoto\_%' ESCAPE '\' THEN
    RAISE EXCEPTION 'Unsupported server-scored test: %', p_test_id;
  END IF;
  ...
  LOOP
    ...
    LOOP
      ...
      IF v_method NOT IN ('radio_exact', 'exact_match', 'ox_match') THEN
        RAISE EXCEPTION 'Unsupported Marugoto scoring method: %', v_method;
      END IF;
  ...
  IF v_section_count = 0 THEN
    RAISE EXCEPTION 'No scoring definition found for test: %', p_test_id;
  END IF;
  RETURN v_scores;
END;
$$;

REVOKE ALL ON FUNCTION calculate_marugoto_scores(VARCHAR, JSONB)
...
SET search_path = public
AS $$
DECLARE v_trainee_id UUID; v_result JSONB;
BEGIN
  SELECT t.id INTO v_trainee_id FROM trainees t
  ...
  SELECT jsonb_object_agg(
    section_type,
    jsonb_build_object('answer_key', answer_key, 'scoring_rules', scoring_rules)
  ) INTO v_result FROM test_sections WHERE test_id = p_test_id;
